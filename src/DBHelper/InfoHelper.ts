...
export default class InfoHelper {
    ...
    public static findInfo = async () => {
        ...
        const lastArticle = articles.filter((item, index) => index < 10)
                                    .map(item => ({ _id: item.id, title: item.title }))
    ...
    public static findAdminInfo = async () => {
        ...
        const says = await Say.find({}).sort({ create_at: -1 }).limit(1);
        const lastSay = says[0];
        const collectNumber = await Collect.count({});
        const collects = await Collect.find({}).sort({ create_at: -1 }).limit(1);
        const lastCollect = collects[0];
        const info: any = await Info.find({}).sort({}).limit(1);
        return {
